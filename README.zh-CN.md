...
- **安全门控晋级**——候选须过三道拦截(指令层对抗扫描、未编辑模板占位符检查、确定性 eval-lite)才能成为技能。该扫描是*规则*门:拦截直白的中英文祈使式与已知混淆手法,不识别任意改写——是兜底,不是保证,批准前请务必人工通读 SKILL.md。
...
| `mine` | 挖掘跨 ≥3 个去重会话复现的任务家族(攒够新会话时由 `status` 与 SessionStart hook 提醒你挖掘)。 |
| `prune [--days N] [--apply]` | 删除超过 TTL（FR-CAP-6）的捕获事件日;默认 dry-run，加 `--apply` 才真删。 |
| `candidate draft/show/approve/reject` | 把挖掘到的家族变成技能:草拟 → 评审 → 三道拦截(门扫描、占位符检查、eval-lite)→ 晋级并落地。 |
...
`materialize`、`doctor --fix` 时被写入(`seed` 只读取、从不修改它)。
...
四条命令会写:`approve`(晋级已评审候选)、`rollback`、`materialize`、`doctor --fix`。`seed` 只把技能读入注册表、
