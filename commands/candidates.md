...
4. `super-skill candidate approve <id>` runs two hard gates (instruction-layer adversarial scan + deterministic eval-lite) and only then promotes: registers an immutable version and materializes it to `~/.claude/skills`. `super-skill candidate reject <id> --reason ...` records a decision without promoting.
